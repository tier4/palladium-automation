...
1. [Git同期設定（オプション）](#git同期設定オプション)
2. [MCP設定（オプション）](#mcp設定オプション)

---

## Git同期設定（オプション）

自動Git同期を有効化すると、ローカルとga53pd01のhornetを自動的に同期・検証します。

### デフォルト動作

**Git同期は無効**（.envファイルが無い場合）
...
# .envファイルを編集してGIT_SYNCを有効化
echo "GIT_SYNC=true" >> .env
```

### Git同期有効時の動作

- ✅ ローカルhornetの未コミット変更をチェック
- ✅ ローカルhornetの未プッシュコミットをチェック
- ✅ ga53pd01でgit pullを自動実行
- ✅ ローカルとリモートのブランチ・コミット一致を確認
- ✅ 不一致時は実行を中止して警告

**Claude Codeによる自動化**:
```
「hornetの変更をコミット＆プッシュして、ga53pd01でビルドを実行して」
```

Claude Codeの実行フロー（Git同期有効時）:
1. `git status`, `git diff`で変更を確認
2. コミットメッセージ案の提示（ユーザーが確認・修正可能）
3. コミット＆プッシュ（ローカルの変更をリモートにプッシュ）
4. Git同期検証（`claude_to_ga53pd01.sh`が自動実行）
   - ✓ 未コミット変更なし
   - ✓ 未プッシュコミットなし
   - ✓ upstream設定済み
5. リモート実行（ga53pd01で`git pull` → スクリプト実行）
...
| シチュエーション | 推奨設定 |
|----------------|---------|
| 頻繁にhornetを編集・コミット・プッシュする | 有効 |
| ga53pd01で独立して作業する | 無効 |
| 試験的な実行のみ行う | 無効 |
